...
print_header "Test Case 4: Attempting to escalate privileges (should be blocked)"
print_step "Creating a RoleBinding to grant 'edit' role in the 'kube-system' namespace..."
ACTION_OUTPUT=$(kubectl --as=system:serviceaccount:operator-test:test-operator apply -f - 2>&1 <<EOF
apiVersion: rbac.authorization.k8s.io/v1
kind: RoleBinding
metadata:
  name: test-operator-escalation
  namespace: kube-system
  labels:
    audit.aquasec.com/type: "security-test"
    audit.aquasec.com/component: "privilege-escalation-attempt"
  annotations:
    audit.aquasec.com/action: "escalate-privileges"
    audit.aquasec.com/description: "Attempt to gain 'edit' access in the 'kube-system' namespace"
subjects:
- kind: ServiceAccount
  name: test-operator
  namespace: operator-test
...
print_action_result "$ACTION_OUTPUT" "Escalate Privileges" "system:serviceaccount:operator-test:test-operator"
